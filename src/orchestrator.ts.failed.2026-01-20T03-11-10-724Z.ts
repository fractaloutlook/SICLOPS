...
- Code Validation Pipeline (TypeScript & ESLint): STABLE - DO NOT MODIFY unless you are implementing a NEW feature.
...
- ⛔ DO NOT run 'git commit' manually. The system auto-commits verified changes.
...
export class Orchestrator {
    ...
    /**
     * Runs a global TypeScript compilation check across the entire project using `npx tsc --noEmit`.
    ...
    private async runTypeScriptValidation(): Promise<{ success: boolean; error?: string }> {
        ...
        try {
            await execAsync('npx tsc --noEmit', { timeout: 60000 }); // Increased timeout for full project
            console.log(`   ✅ Global TypeScript validation passed!`);
            return { success: true };
        } catch (compileError: any) {
            const errorMsg = compileError.stderr || compileError.stdout || compileError.message;
            console.error(`   ❌ Global TypeScript compilation failed:`);
    ...
    private async runESLintValidation(): Promise<{ success: boolean; error?: string }> {
        ...
        // ESLint is already imported globally at line 20
        ...
        try {
            ...
            let hasIssues = false; // Check for both errors and warnings
            ...
            if (hasIssues) {
                console.error(`   ❌ ESLint validation failed with errors/warnings:\n${resultText}`);
            ...
            const errorMsg = lintError.message || 'Unknown ESLint error';
            console.error(`   ❌ ESLint validation encountered a critical error:`);
            console.error(`   ${errorMsg.substring(0, 500)}`);
            return { success: false, error: errorMsg };
        }
    }
...
Reference: docs/SYSTEM_CAPABILITIES.md and docs/AGENT_GUIDE.md for full details.`;
        ...
        // Run global code validations after all agents have made their changes.
        // The pipeline is: TypeScript compilation -> ESLint validation.
        // If any step fails, subsequent steps (including tests and auto-commit) are skipped.
        let globalValidationPassed = true; // Renamed flag to be more generic for all code validations
        const changedFiles = extractChangedFiles(projectFile.history);
        if (changedFiles.some(file => file.endsWith('.ts') || file.endsWith('.js'))) { // Only run if TS/JS files were changed
            const tsValidationResult = await this.runTypeScriptValidation();
            if (!tsValidationResult.success) {
                globalValidationPassed = false;
                console.log(`
❌ Global TypeScript validation failed. Skipping further validation and auto-commit.
`);
            }

            // Run ESLint validation if TypeScript validation passed
            if (globalValidationPassed) {
                const eslintValidationResult = await this.runESLintValidation();
                if (!eslintValidationResult.success) {
                    globalValidationPassed = false;
                    console.log(`
❌ ESLint validation failed. Skipping tests and auto-commit.
`);
                }
            }
        }

        // Run tests on changed files (only if all global validations passed)
        if (globalValidationPassed && changedFiles.length > 0) {
    ...
    private getNextAgentInWorkflow(currentAgentName: string, availableAgents: string[]): BaseAgent | null {
        ...
        if (currentIndex === -1) {
            ...
            for (const agentName of AGENT_WORKFLOW_ORDER) {
                // If passive mode is on, or if Human is disabled, skip Tim (Human) in the rotation
                if ((this.config.passiveHuman || this.config.noHuman) && agentName === 'Tim') continue;
