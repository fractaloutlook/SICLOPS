...
describe('SharedMemoryCache', () => {
  ...
  describe('Sensitive Bucket Protection', () => {
    ...
    test('CRITICAL: should allow manual eviction of sensitive entries', () => {
      // Security requirement: Sensitive data must be protected from auto-eviction
      // BUT must be manually evictable when agents need to remove secrets/credentials
      const sensitiveData = 'API_KEY=secret123';
      cache.store('credentials', sensitiveData, 'sensitive');
      
      // Verify it's stored
      expect(cache.retrieve('credentials')).toBe(sensitiveData);
      
      // Manual eviction should succeed
      const evicted = cache.evict('credentials');
      expect(evicted).toBe(true);
      
      // Entry should be gone
      expect(cache.retrieve('credentials')).toBeNull();
      
      const stats = cache.getStats();
      expect(stats.bucketStats.sensitive.entries).toBe(0);
